...
def prep_data_for_booster(audio_input_file, audio_names_file, image_input_file, out_file):
...
def read_data_from_paths(aud_path: str, aud_name_path: str, img_path: str) -> list:
    ...
    aws_list = calculate_aws_prediction_list_training(aws_contents) if False else calculate_aws_prediction_list(
        aws_contents)


    ret_list = merge_lists(aud_dict, aws_list, False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--audio_input_file', '-a', type=str, default='./pipeline_helper/audio_nn_predictions.csv',
                        help='Audio predictions')
    parser.add_argument('--audio_names_file', '-n', type=str, default='./pipeline_helper/audio_nn_prediction_names.csv',
                        help='Audio names')
    parser.add_argument('--image_input_file', '-i', type=str, default='./pipeline_helper/aws_results.json', help='Image JSON file')
    parser.add_argument('--out_file', '-o', type=str, default='./pipeline_helper/results_for_lgbm.json', help="File for output")
    args = parser.parse_args()

    prep_data_for_booster(args.audio_input_file, args.audio_names_file, args.image_input_file, args.out_file)